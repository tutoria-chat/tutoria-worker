"""
xAI Service - OpenAI-compatible API via AsyncOpenAI

Provides access to xAI Grok models (grok-3-mini, grok-3, etc.)
through the OpenAI-compatible chat completions endpoint at api.x.ai.
...
class XAIService:
    """
    AI Service using xAI's OpenAI-compatible API (Grok models).

    - Uses openai.AsyncOpenAI with custom base_url pointed at xAI
    ...
    def __init__(
        ...
        elif module.ai_model and module.ai_model.model_name and getattr(module.ai_model, 'provider', '').lower() == 'xai':
            self.model_name = module.ai_model.model_name
        else:
            self.model_name = DEFAULT_XAI_MODEL
        ...
        if not resolved_key and db:
            try:
                ...
                result = self._key_manager.get_next_key("xai")
                if result:
                    self._key_id, resolved_key = result
                    logger.info(f"Using DB-managed xAI key (id={self._key_id})")
            except Exception as e:
                logger.warning(f"Failed to load DB-managed xAI key: {e}")

        if not resolved_key:
            resolved_key = settings.XAI_API_KEY

        if not resolved_key:
            logger.error("No xAI API key available (env, DB, or explicit)")
            raise ValueError("XAI_API_KEY is required for XAIService")

        # Initialize AsyncOpenAI with xAI base URL
        ...
        logger.info(
            f"Initialized XAIService (model={self.model_name}, "
            f"base_url={settings.XAI_BASE_URL})"
        )
    ...
    async def _call_with_retry(self, coro_factory, description: str = "xAI API call"):
        """
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if self._key_manager and self._key_id:
                    try:
                        self._key_manager.mark_key_success("xai", self._key_id)
                    except Exception:
                        pass
                return result
                ...
                if not retryable or attempt == max_retries:
                    ...
                    if self._key_manager and self._key_id:
                        try:
                            self._key_manager.mark_key_failed("xai", self._key_id)
                        except Exception:
                            pass
                    raise
    ...
    async def answer_question_with_history(
        ...
        try:
            logger.info(
                f"xAI answer_question_with_history called "
            ...
            response = await self._call_with_retry(
                _make_request, description="xAI chat completion"
            )
            ...
            logger.error(f"xAI answer_question_with_history error: {e}")
            return {
                "response": f"Error communicating with xAI: {str(e)}",
                "tokens": {
                    "prompt_tokens": 0,
                    "completion_tokens": 0,
                    "total_tokens": 0,
