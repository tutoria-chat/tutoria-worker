...
def _parse_grading_response(text: str, questions: list[dict]) -> list[dict]:
    ...
    for q in questions:
        ...
        if slot in by_slot:
            item = by_slot[slot]
            # Compliance: no grade is parsed or produced — only formative feedback.
            # Accept 'feedback' (current) or 'comment' (legacy) as the text.
            comment = str(item.get("feedback", item.get("comment", ""))).strip() or "—"
        else:
            comment = "Não avaliado"
        results.append({"slot": slot, "comment": comment})
...
class GradingService:
    ...
    async def _call_ai_for_student(
        ...
        # Compliance (CNE Guidelines for AI in Education, 2026): the AI must NOT
        # correct, evaluate or suggest a grade on discursive answers — not even a
        # draft for the teacher. It only produces formative feedback and, when a
        # rubric is given, a qualitative per-criterion attendance analysis. The
        # grade is the teacher's exclusive responsibility.
        system_prompt = (
            "You are a PEDAGOGICAL CORRECTION ASSISTANT — NOT a grader. You help the teacher "
            "by writing FORMATIVE, CONSTRUCTIVE feedback on each student answer, and, when "
            "grading criteria (a rubric) are provided, a per-criterion analysis of whether each "
            "criterion was met. "
            "You MUST NOT assign, suggest, imply or mention ANY grade, score, mark, number of "
            "points, percentage or letter — not as a value, a range, or a draft. The grade is the "
            "teacher's exclusive decision. Do not say things like 'this deserves', 'worth X', "
            "'X out of Y' or similar. "
            "If a 'reference_answer' (gabarito) is provided, use it ONLY to point out, formatively, "
            "what the answer addressed well and what it missed — never to compute a score. Use the "
            "course content only as supporting context; when images are attached, use them to "
            "interpret visual questions or answers. "
            "For each rubric criterion, state qualitatively whether it was 'atendido', "
            "'parcialmente atendido' or 'não atendido', with a short justification — no numbers or weights. "
        ...
        user_prompt = (
            ...
            f"{criteria_section}"
            f"{image_note}"
            f"Analyse the following student answers and write formative feedback (and, when a "
            f"rubric is given, a per-criterion attended/partially/not-attended analysis) WITHOUT "
            f"any grade:\n{questions_json}\n\n"
            "Return a JSON array:\n"
            '[{"slot": <number>, "feedback": "<formative feedback; no grade, score or number>"}]\n'
