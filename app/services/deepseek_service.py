"""
DeepSeek Service - OpenAI-compatible API via AsyncOpenAI

Provides access to DeepSeek models (deepseek-chat, deepseek-reasoner, etc.)
through the OpenAI-compatible chat completions endpoint.

Uses the same interface as AnthropicService/BedrockService for drop-in
provider swapping in the widget chat pipeline.
"""
import asyncio
import logging
import random
...
class DeepSeekService:
    """
    AI Service using DeepSeek's OpenAI-compatible API.

    - Uses openai.AsyncOpenAI with custom base_url pointed at DeepSeek
    ...
    def __init__(
        ...
        elif module.ai_model and module.ai_model.model_name and getattr(module.ai_model, 'provider', '').lower() == 'deepseek':
        ...
        if not resolved_key and settings.USE_DB_PROVIDER_KEYS and db:
            try:
                ...
                result = self._key_manager.get_next_key("deepseek")
                if result:
                    self._key_id, resolved_key = result
                    logger.info(f"Using DB-managed DeepSeek key (id={self._key_id})")
            except Exception as e:
                logger.warning(f"Failed to load DB-managed DeepSeek key: {e}")

        if not resolved_key:
            resolved_key = settings.DEEPSEEK_API_KEY

        if not resolved_key:
            logger.error("No DeepSeek API key available (env, DB, or explicit)")
            raise ValueError("DEEPSEEK_API_KEY is required for DeepSeekService")

        # Initialize AsyncOpenAI with DeepSeek base URL
        ...
        logger.info(
            f"Initialized DeepSeekService (model={self.model_name}, "
    ...
    async def _call_with_retry(self, coro_factory, description: str = "DeepSeek API call"):
        """
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if self._key_manager and self._key_id:
                    try:
                        self._key_manager.mark_key_success("deepseek", self._key_id)
                    except Exception:
                        pass
                return result
                ...
                if not retryable or attempt == max_retries:
                    ...
                    if self._key_manager and self._key_id:
                        try:
                            self._key_manager.mark_key_failed("deepseek", self._key_id)
                        except Exception:
                            pass
                    raise
    ...
    async def answer_question_with_history(
        ...
        try:
            logger.info(
                f"DeepSeek answer_question_with_history called "
            ...
            response = await self._call_with_retry(
                _make_request, description="DeepSeek chat completion"
            )
            ...
            logger.error(f"DeepSeek answer_question_with_history error: {e}")
            return {
                "response": f"Error communicating with DeepSeek: {str(e)}",
                "tokens": {
                    "prompt_tokens": 0,
                    "completion_tokens": 0,
                    "total_tokens": 0,
